...
def allocate_fft_resources(config: FFTConfig) -> FFTResources:
    inline_batch_x = allocate_inline_batches(config.batch_x_count, config.batch_threads, config.sdata_allocation, vd.get_context().max_workgroup_size[0])
    
    inline_batch_z = allocate_inline_batches(config.batch_z_count, config.batch_threads * inline_batch_x, config.sdata_allocation, vd.get_context().max_workgroup_size[2])

    sdata_buffer = vc.shared_buffer(vc.c64, config.sdata_allocation * inline_batch_z * inline_batch_x, "sdata")
    sdata_offset = None

    if inline_batch_z > 1 or inline_batch_x > 1:
        sdata_offset = vc.new_uint(vc.local_invocation().z * inline_batch_x * config.N + vc.local_invocation().x * config.N, var_name="sdata_offset")
    ...
    resources = FFTResources(
        ...
        shared_memory_size=config.N * inline_batch_z * inline_batch_x * vd.complex64.item_size,
        local_size=(inline_batch_x, config.batch_threads, inline_batch_z)
    )
