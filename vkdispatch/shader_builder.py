...
class ShaderBuilder:
    ...
    def make_var(self, var_type: vd.dtype, var_name: str = None) -> vd.ShaderVariable:
    ...
    def new(self, var_type: vd.dtype, var_name: str = None) -> vd.ShaderVariable:
        new_var = self.make_var(var_type, var_name)
        self.append_contents(f"{var_type.glsl_type} {new_var};\n")
        return new_var

    def dynamic_buffer(
        self, var_type: vd.dtype, var_name: str = None
    ) -> vd.ShaderVariable:
    ...
    def shared_buffer(
        self, var_type: vd.dtype, size: int, var_name: str = None
    ) -> vd.ShaderVariable:
    ...
    def if_statement(self, arg: vd.ShaderVariable) -> None:
        self.append_contents(f"if({arg}) {'{'}\n")
        self.scope_num += 1

    def if_any(self, *args: List[vd.ShaderVariable]) -> None:
        self.append_contents(f"if({' || '.join([str(elem) for elem in args])}) {'{'}\n")
        self.scope_num += 1

    def if_all(self, *args: List[vd.ShaderVariable]) -> None:
        self.append_contents(f"if({' && '.join([str(elem) for elem in args])}) {'{'}\n")
        self.scope_num += 1
    ...
    def ceil(self, arg: vd.ShaderVariable) -> vd.ShaderVariable:
        return self.make_var(arg.var_type, f"ceil({arg})")

    def exp(self, arg: vd.ShaderVariable) -> vd.ShaderVariable:
        return self.make_var(arg.var_type, f"exp({arg})")

    def sin(self, arg: vd.ShaderVariable) -> vd.ShaderVariable:
        return self.make_var(arg.var_type, f"sin({arg})")

    def cos(self, arg: vd.ShaderVariable) -> vd.ShaderVariable:
        return self.make_var(arg.var_type, f"cos({arg})")

    def sqrt(self, arg: vd.ShaderVariable) -> vd.ShaderVariable:
        return self.make_var(arg.var_type, f"sqrt({arg})")

    def atomic_add(
        self, arg1: vd.ShaderVariable, arg2: vd.ShaderVariable
    ) -> vd.ShaderVariable:
    ...
    def subgroup_add(
        self,
        arg1: vd.ShaderVariable,
    ) -> vd.ShaderVariable:
        return self.make_var(arg1.var_type, f"subgroupAdd({arg1})")

    def float_bits_to_int(self, arg: vd.ShaderVariable) -> vd.ShaderVariable:
        return self.make_var(vd.int32, f"floatBitsToInt({arg})")

    def int_bits_to_float(self, arg: vd.ShaderVariable) -> vd.ShaderVariable:
        return self.make_var(vd.float32, f"intBitsToFloat({arg})")

    def print(self, *args: Union[vd.ShaderVariable, str]) -> None:
        args_list = []
