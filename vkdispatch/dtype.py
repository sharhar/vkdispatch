...
class dtype:  # TODO: Adhere to python class naming conventions
    def __init__(self, name: str, item_size: int, glsl_type: str, structure: dtype_structure, child_count: int, format_str: str, parent: 'dtype' = None, is_complex: bool = False) -> None:
        ...
        self.scalar = self if self.structure == dtype_structure.DATA_STRUCTURE_SCALAR else self.parent.scalar

        self.child_count = child_count
        self.total_count = 1 if parent is None else child_count * self.parent.total_count
        self.scalar_count = 1 if self.structure == dtype_structure.DATA_STRUCTURE_SCALAR else child_count * self.parent.scalar_count
        self._true_shape = () if parent is None else (self.child_count, *self.parent._true_shape)
        self.shape = (1,) if parent is None else self._true_shape

        self._true_numpy_shape = () if self.structure == dtype_structure.DATA_STRUCTURE_SCALAR else (self.child_count, *self.parent._true_numpy_shape)
        self.numpy_shape = (1,) if self.structure == dtype_structure.DATA_STRUCTURE_SCALAR else self._true_numpy_shape
