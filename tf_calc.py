...
def get_circPowerFromEta0Deg(eta0_deg, HT, l_wlen, NA):
    ...
    return (
        ...
        * hbar
        * c**2
        / (np.sqrt(2 / np.pi**3) * alpha * e_wlen * 1e-10 * l_wlen * 1e-10 * NA)
    )  # [W]
...
def get_sigmaE(HT):
    ...
    return (
        ...
        * np.pi
        / (wlen * HT)
        * ((e_mass * J_per_eV) + e_charge_SI * HT)
        / (2 * e_mass * J_per_eV + e_charge_SI * HT)
    )  # [rad/(V*A)]
...
def get_defocusSpread(Cc, HT, energy_spread_std, accel_voltage_std, lens_current_std):
    ...
    return Cc * np.sqrt(
        (epsilon * energy_spread_std / HT) ** 2
        + (epsilon * accel_voltage_std) ** 2
        + 4 * lens_current_std**2
...
class LaserPP(PhasePlate):
    ...
    def __init__(self, **kwargs):
        ...
        self.long_phi_deg = (
            0  # longitudinal shift [deg], complementary to long_offset (0: antinode)
        )
...
class Microscope:
    ...
    def get_derivParams(self):
        ...
        self.energy_spread_std = get_sigmaFromFWHM(
            self.energy_spread_fwhm
        )  # energy spread [std]
        self.defocus_spread = get_defocusSpread(
            self.Cc,
            self.HT,
            self.energy_spread_std,
            self.accel_voltage_std,
            self.lens_current_std,
        )  # defocus spread (std) [A]
...
def get_laserCoordsL(pp, sco: Microscope, sgrid):
    ...
    Rr_rot = -Sc_phys * np.sin(np.deg2rad(pp.rc_ang_deg)) + Sr_phys * np.cos(
        np.deg2rad(pp.rc_ang_deg)
    )  # [A]
    Rc_rot = Sc_phys * np.cos(np.deg2rad(pp.rc_ang_deg)) + Sr_phys * np.sin(
        np.deg2rad(pp.rc_ang_deg)
    )  # [A]
...
def get_laserPhase(pp, sco: Microscope, sgrid):
    ...
    return (
        pp.eta0_rad
        / 2
        * np.exp(-2 * Lr**2 / (1 + Lc**2))
        / np.sqrt(1 + Lc**2)
        * (
            1
            + (1 - 2 * sco.beta**2 * (np.cos(np.deg2rad(pp.pol_ang_deg))) ** 2)
...
def get_phasePlatePhase(pp, sco: Microscope, srad, sgrid):
    ...
    if pp.mode == "off":
        ...
        if "eta0_deg" in pp.__dict__.keys():
            pp.circ_power = get_circPowerFromEta0Deg(
                pp.eta0_deg, sco.HT, pp.wlen, pp.NA
            )  # [W]
            pp.eta0_rad = np.deg2rad(pp.eta0_deg)  # [rad]
            eta0_tot_deg += pp.eta0_deg  # [deg]
        elif "circ_power" in pp.__dict__.keys():
            eta0_deg = get_eta0DegFromCircPower(
                pp.circ_power, sco.HT, pp.wlen, pp.NA
            )  # [deg]
        ...
        if pp.pol_ang_deg == "rra":
            ...
            print(
                "Set laser polarization to reversal angle ({:.2f} deg)".format(
                    pp.pol_ang_deg
                )
            )
...
def prepareTF(shape, pix_size, mode, upsample_factor=1) -> None:
    ...
    sco = Microscope(
        enable_coh_env=True, enable_coh_johnson=False, Cs=2.7e7, Cc=2.7e7, f_OL=3.5e7
    )
    ...
    mag_pre = np.exp(-(sigma_f**2) * V2**2 / 2) * get_johnsonEnvelope(
        srad, sco.johnson_var
    )
    ...
    V1_r_adder = (
        ...
        * np.pi
        * (
            sco.astig_z * sco.wlen * srad * np.sin(sphi - 2 * np.deg2rad(sco.astig_deg))
            + sco.Cs * sco.wlen**3 * srad**3 * np.sin(sphi)
        )
    )
    ...
    V1_c_adder = (
        ...
        * np.pi
        * (
            sco.Cs * sco.wlen**3 * srad**3 * np.cos(sphi)
            - sco.astig_z
            * sco.wlen
            * srad
            * np.cos(sphi - 2 * np.deg2rad(sco.astig_deg))
        )
    )
