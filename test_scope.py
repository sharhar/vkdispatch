...
class Scope:
    def __init__(self, template_shape, tf_data: typing.Tuple[np.ndarray], sigma_e: float, amp_ratio: float) -> None:
        ...
        self.potential_to_wave = potential_to_wave
        self.mult_by_mask = mult_by_mask
        self.apply_transfer_function = apply_transfer_function
        self.calc_sums = calc_sums
    ...
    def record(self, cmd_list: vd.CommandList, work_buffer: vd.Buffer, defocus: float = None):
        self.potential_to_wave[self.exec_size, cmd_list](work_buffer)

        vd.fft[cmd_list](work_buffer)
        self.mult_by_mask[work_buffer.size, cmd_list](work_buffer)
        
        if defocus is not None:
            self.apply_transfer_function[work_buffer.size, cmd_list](work_buffer, self.tf_data_buffer, defocus=defocus)
        else:
            self.defocus = self.apply_transfer_function[work_buffer.size, cmd_list](work_buffer, self.tf_data_buffer)
        
        vd.ifft[cmd_list](work_buffer)

        self.sum_buff = self.calc_sums[work_buffer.size, cmd_list](work_buffer) # The reduction returns a buffer with the result in the first value
        self.normalize_image[work_buffer.size, cmd_list](work_buffer, self.sum_buff)
